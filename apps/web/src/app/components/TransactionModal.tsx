...
const isSalaryCategoryName = (name: string) => {
  const normalized = name.trim().toLowerCase();
  return SALARY_CATEGORY_KEYWORDS.some((keyword) => normalized.includes(keyword.toLowerCase()));
};

export const filterTransactionCategoriesByType = (
  categories: AccountGroup['categories'],
  type: 'income' | 'expense' | 'transfer'
) => {
  if (type === 'transfer') {
    return [];
  ...
  return categories.filter((category) => {
    ...
    // Salary-like categories should only be selectable under income.
    if (type === 'expense' && isSalaryCategoryName(category.name)) {
      return false;
    }
...
export const filterAccountGroupsByTransactionType = (
  groups: AccountGroup[],
  type: 'income' | 'expense' | 'transfer'
) => {
  if (type === 'transfer') {
  ...
  return groups.filter(
    (group) => filterTransactionCategoriesByType(group.categories || [], type).length > 0
  );
};

export const computeStockAmount = (sharesInput: string, unitPriceInput: string): string => {
  const shares = Number.parseFloat(sharesInput);
  const unitPrice = Number.parseFloat(unitPriceInput);

  if (!Number.isFinite(shares) || !Number.isFinite(unitPrice) || shares <= 0 || unitPrice <= 0) {
    return '';
  }

  const total = Math.round(shares * unitPrice * 100) / 100;
...
export const TransactionModal: React.FC<TransactionModalProps> = ({
  ...
  useEffect(() => {
    const currentGroup = selectableAccountGroups.find((g) => g.id === accountGroupId);
    if (currentGroup && currentGroup.categories) {
      const filtered = filterTransactionCategoriesByType(currentGroup.categories, type);
  ...
  }, [accountGroupId, type, selectableAccountGroups, category]);
  ...
  useEffect(() => {
    ...
    if (selectableAccountGroups.length === 0) {
      setAccountGroupId('');
      setCategory('');
      return;
    }

    const currentGroupStillValid = selectableAccountGroups.some((g) => g.id === accountGroupId);
    if (!currentGroupStillValid) {
      setAccountGroupId(selectableAccountGroups[0].id);
    }
        ...
        <div>
          ...
          {isEditingInstallment ? (
            ...
            <CustomSelect
              ...
              options={selectableAccountGroups.map((group) => ({
                value: group.id,
                label: group.name,
                icon: group.emoji,
              }))}
            ...
            <CustomSelect
              ...
              options={(
                filterTransactionCategoriesByType(
                  selectableAccountGroups.find((g) => g.id === accountGroupId)?.categories || [],
                  type
                )
              ).map((cat) => ({
                value: cat.name,
